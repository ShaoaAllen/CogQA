...
import re
import pdb
from transformers import AlbertTokenizer, RobertaConfig, RobertaTokenizer, XLNetTokenizer, XLNetModel
...
from transformers.models.roberta.modeling_roberta import RobertaEmbeddings, RobertaPreTrainedModel, RobertaModel, RobertaEncoder, RobertaPooler
...
class RobertaModelPlus(RobertaModel):
...
class RobertaForMultiHopQuestionAnswering(RobertaPreTrainedModel):
    def __init__(self, config):
        super(RobertaForMultiHopQuestionAnswering, self).__init__(config)
        self.roberta = RobertaModelPlus(config)
        self.qa_outputs = nn.Linear(config.hidden_size, 4)
    ...
    def forward(
        ...
        batch_size = input_ids.size()[0]
        device = input_ids.get_device() if input_ids.is_cuda else torch.device("cpu")
        sequence_output, hidden_output = self.roberta(
            input_ids, token_type_ids, attention_mask
        )
        semantics = hidden_output[:, 0]
        # Some shapes: sequence_output [batch_size, max_length, hidden_size], pooled_output [batch_size, hidden_size]
