...
from model import BertForMultiHopQuestionAnswering
...
from run_fail_store.lstm_model import *
from lstm.utils import *
#from lstm.evaluate import *
from lstm.dataloader import *
...
def main(output_model_file = './models/pytorch_model.bin', load = False, mode = 'tensors', batch_size = 7,
    ...
    if load:
        ...
        #num_epochs = int(sys.argv[1:][-1])
        if len(sys.argv) not in [7, 8]:
            sys.exit("Usage: %s model char_to_idx word_to_idx tag_to_idx training_data (validation_data) num_epoch" %
                     sys.argv[0])
        batch, cti, wti, itt = load_data(sys.argv[1:])
        model2 = rnn_crf(len(cti), len(wti), len(itt))
