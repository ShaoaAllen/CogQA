...
from GraphSAGE_model import BertForMultiHopQuestionAnswering, CognitiveGNN
...
def main(output_model_file = './models/pytorch_model.bin', load = False, mode = 'tensors', batch_size = 7,
    ...
    if load:
        ...
        model2 = GraphSage(model1.config.hidden_size,config['setting.num_layers'], features.size(1), config['setting.hidden_emb_size'],
                              features, getattr(dataCenter, ds + '_adj_lists'), device, gcn=args.gcn,
        ...
        #model1 = RobertaForMultiHopQuestionAnswering.from_pretrained(ROBERTA_MODEL, cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(-1))
        model2 = GraphSage(config['setting.num_layers'], features.size(1), config['setting.hidden_emb_size'],
                              features, getattr(dataCenter, ds + '_adj_lists'), device, gcn=args.gcn,
                              agg_func=args.agg_func)
        model2.to(device)
