...
from GAT_model import BertForMultiHopQuestionAnswering, GAT, SpGAT
...
from GAT.utils import load_data
import argparse
...
def main(output_model_file = './models/pytorch_model.bin', load = True, mode = 'tensors', batch_size = 7,
    ...
    if load:
        ...
        if args.sparse:
            model2 = SpGAT(nfeat=features.shape[1],
                          nhid=args.hidden,
            ...
            model2 = GAT(nfeat=features.shape[1],
                        nhid=args.hidden,
                        nclass=int(labels.max()) + 1,
                        dropout=args.dropout,
                        nheads=args.nb_heads,
